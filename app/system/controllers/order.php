    ...
    case 'package':
        ...
        if (!empty($_SESSION['nux-router'])) {
            if ($_SESSION['nux-router'] == 'radius') {
                ...
                $nuxr = $_SESSION['nux-router'];
                if (ctype_digit((string)$nuxr)) {
                    $routers = ORM::for_table('tbl_routers')->where('id', $nuxr)->find_many();
                } else {
                    $routers = ORM::for_table('tbl_routers')->where('name', $nuxr)->find_many();
                }
                $rs = [];
